	...
	return {
		...
		onSampleClick: function(url_complete) {
			...
			for(var i=0; i<ontologies.length; i++){
				var ontologyToLoad = ontologies[i]
				fetcher.nowOrWhenFetched(ontologyToLoad, function(ok, body, xhr) {
					if (!ok) {
						console.log("Oops, something happened and couldn't fetch data");
					} else {
						// console.log("Loaded " + ontologyToLoad)
						labels_data.push(store.statementsMatching(undefined,$rdf.sym('http://www.w3.org/2000/01/rdf-schema#label'),undefined))
					}
					
					for(var i = 0; i<labels_data.length; i++){
						for(var y = 0; y<labels_data[i].length; y++){
							if(labels_data[i][y].object.lang){
								if(labels_data[i][y].object.lang == 'en'){
									var sub = labels_data[i][y].subject.value
									var obj = labels_data[i][y].object.value
							...
							fetcher.nowOrWhenFetched(Dbpedia_sameAs, function(ok, body, xhr) {
								if (!ok) {
									...
									for(var i=0; i<unique_prop.length; i++){       	
										...
										var propertyLabel = getPropertyLabel(prop,propertiesLabels)
										var objects = store.each(s,$rdf.sym(prop),undefined);
